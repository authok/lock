import authok from '@authok/authok-js';
import qs from 'qs';
import CordovaAuthokPlugin from '@authok/authok-js/dist/cordova-authok-plugin.min.js';
import {
  normalizeError,
  loginCallback,
  normalizeAuthParams,
...
class AuthokAPIClient {
  constructor(lockID, clientID, domain, opts) {
    ...
    this.client = new authok.WebAuth({
      clientID: clientID,
      domain: domain,
      ...
      plugins: opts.plugins || [new CordovaAuthokPlugin()],
  ...
  getTelemetryInfo(telemetryOverride) {
    let telemetry;
    const { authokClient } = qs.parse(window.location.search.substr(1));
    let ulpTelemetry = authokClient && JSON.parse(atob(authokClient));
    if (this.isUniversalLogin && ulpTelemetry) {
      telemetry = {
        ...
        env: {
          ...
          ['authok.js-ulp']: authok.version.raw
        }
      };
    }
    if (this.isUniversalLogin && !ulpTelemetry) {
...
export default AuthokAPIClient;
