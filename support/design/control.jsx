    ...
    return (
      <div className="control">
        <div>
          ...
          <ResultSelect
            selected={this.state.startPasswordless.response}
            onChange={::this.handleStartPasswodlessResponseChange}
          />

          <h3>logIn</h3>
          <ResultSelect
            selected={this.state.logIn.response}
            onChange={::this.handleLogInResponseChange}
          />

          <h3>signUp</h3>
          <ResultSelect
            selected={this.state.signUp.response}
            onChange={::this.handleSignUpResponseChange}
          />

          <h3>resetPassword</h3>
          <ResultSelect
            selected={this.state.resetPassword.response}
...
class Snapshot extends React.Component {
  constructor(props) {
    super(props);
    this.state = { snapshot: JSON.stringify(getState().toJS()) };
  }

  componentDidMount() {
    subscribe('design-tools.snapshot', (key, oldState, newState) => {
      if (newState) {
        this.setState({ snapshot: JSON.stringify(newState.toJS()) });
      }
    });
  }
...
window.loadSnapshot = function(str) {
  swap(state => state.set('lock', Immutable.fromJS(JSON.parse(str).lock)));
};
