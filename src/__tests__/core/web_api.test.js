import AuthokWebApi from '../../core/web_api';

describe('AuthokWebApi', () => {
  let originalWindow;

  const LOCK_ID = 'lock-id';
  const CLIENT_ID = 'client-id';
  const DEFAULT_DOMAIN = 'test.com';
  const client = () => AuthokWebApi.clients[LOCK_ID];
  ...
  describe('setupClient', () => {
    it('sets the correct options when is on the hosted login page', () => {
      ...
      AuthokWebApi.setupClient(LOCK_ID, CLIENT_ID, DEFAULT_DOMAIN, { redirect: true });
    ...
    it('sets redirect: true when on the same origin as the specified domain', () => {
      ...
      AuthokWebApi.setupClient(LOCK_ID, CLIENT_ID, DEFAULT_DOMAIN, {});
