...
function bindEvents() {
  ...
  // its close button.
  $('body').on('click', '.authok-lock-close-button', function(ev) {
    ev.preventDefault();
  ...
  $('body').on('keydown', function(ev) {
    if (ev.keyCode === 27 && $('.authok-lock-opened .authok-lock-close-button').length) {
      showLockHandler();
    }
  });
}
...
function showLockHandler(ev) {
  ...
  try {
    if ($('.authok-lock').length) {
      window.lock.close();
    }

    window.lock = new AuthokLockPasswordless(clientID, domain);
    ...
    setTimeout(function() {
      remember.except('.authok-lock-input');
    }, 0);
  } catch (e) {
