...
function extractUIOptions(id, options) {
  ...
  return new Immutable.fromJS({
    containerID: options.container || `authok-lock-container-${id}`,
...
export function extractTenantBaseUrlOption(opts, domain) {
  if (opts.configurationBaseUrl && typeof opts.configurationBaseUrl === 'string') {
    if (opts.overrides && opts.overrides.__tenant) {
      // When using a custom domain and a configuration URL hosted in authok's cdn
...
export function captcha(m) {
  ...
  // https://github.com/authok/lock/blob/82f56187698528699478bd429858cf91e387763c/src/__tests__/engine/classic/sign_up_pane.test.jsx#L28
  if (typeof m !== 'object') {
    return;
  }
