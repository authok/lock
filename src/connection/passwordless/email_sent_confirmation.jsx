...
class ResendLink extends React.Component {
  render() {
    ...
    return (
      <a className="authok-lock-resend-link" href="#" onClick={onClick}>
...
class Resend extends React.Component {
  render() {
    ...
    const resendingLabel = m.resendOngoing(lock) && (
      <a className="authok-lock-resend-link">{labels.resending}</a>
    );

    const resendSuccessLabel = m.resendSuccess(lock) && (
      <span className="authok-lock-sent-label">{labels.sent}</span>
    );

    const resendFailedLabel = m.resendFailed(lock) && (
      <span className="authok-lock-sent-failed-label">{labels.failed}</span>
    );
