...
class ContainerManager {
    ...
    if (!container && shouldAppend) {
      ...
      container.id = id;
      container.className = 'authok-lock-container';
...
class Renderer {
  ...
  render(containerId, props) {
    ...
    let component = component = ReactDOM.createRoot(container).render(<Container {...props} />);
    // let component = ReactDOM.render(<Container {...props} />, container);
  ...
  unmount(containerId) {
    ...
    if (this.modals[containerId]) {
      ...
      CSSCore.removeClass(window.document.getElementsByTagName('html')[0], 'authok-lock-html');
    }
  }
}
