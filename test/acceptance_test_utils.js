...
const CURRENT_CRED_PANE_SELECTOR =
  '.authok-lock-cred-pane:not(.horizontal-fade-exit):not(.reverse-horizontal-fade-exit)';
export const CRED_PANE_DELAY = 1850;

// We also perform an animated transition with auxiliary panes.
const AUXILIARY_PANE_SELECTOR_SUFFIX = ':not(.slide-exit)';
...
export function constructLock(cid = 'a', domain = 'a') {
  return new AuthokLockPasswordless(cid, domain);
}
...
export function isOpened(lock) {
  ...
  return node && CSSCore.hasClass(node, 'authok-lock-opened');
}

export function closeLock(lock) {
  const node = q(lock, '.authok-lock-close-button');
  if (!node) {
...
export function qInput(lock, name, ensure = false) {
  ...
  const input = q(lock, `${CURRENT_CRED_PANE_SELECTOR} .authok-lock-input-${name} input`);
  if (ensure && !input) {
...
export function isInputInvalid(lock, name) {
  ...
  if (!node) {
    throw new Error(
      `Unable to tell whether '${name}' input is invalid: can't find container element responsible to indicate it`
    );
  }
  return CSSCore.hasClass(node, 'authok-lock-error');
}
...
export function isLoading(lock) {
  ...
  return CSSCore.hasClass(node, 'authok-lock-mode-loading');
}
...
export function isSomethingWrong(lock, expectedMessage) {
  const node = q(lock, '.authok-global-grobal-error');
  if (!node) {
    return false;
  }
  ...
  return typeof expectedMessage.test === 'function'
    ? expectedMessage.test(acutalMessage)
    : expectedMessage === acutalMessage;
}
...
export function isShowingConfirmation(lock) {
  return !!q(lock, '.authok-lock-confirmation:not(.slide-exit)');
}

function qResendLink(lock, ensure = false) {
  const node = q(lock, '.authok-lock-resend-link');
  if (ensure && !node) {
...
export function hasLinkBeenResent(lock) {
  const node = q(lock, '.authok-lock-sent-label');
  if (!node) {
    throw new Error(
      `Unable to tell whether the link has been resent: can't find the element responsible to indicate it`
    );
  }
...
export function hasResendingFailed(lock) {
  const node = q(lock, '.authok-lock-sent-failed-label');
  if (!node) {
    throw new Error(
      `Unable to tell whether the resending link has failed: can't find the element responsible to indicate it`
    );
  }
...
export function isShowingLocationSelector(lock) {
  return isShowingAuxiliaryPane(lock, '.authok-lock-select-country');
}
...
export function qLocations(lock) {
  ...
  return q(
    lock,
    `.authok-lock-select-country${AUXILIARY_PANE_SELECTOR_SUFFIX} .authok-lock-list-code > ul > li`,
    true
  );
}
...
export function filterLocations(lock, value) {
  ...
  const input = q(
    lock,
    `.authok-lock-select-country${AUXILIARY_PANE_SELECTOR_SUFFIX} .authok-lock-search input`
  );
  if (!input) {
